...
function renderRespondentTable(data) {
    ...
    data.forEach(row => {
        ...
        tr.innerHTML = `<td>${row['First name']} ${row['Last name']}</td><td>${row['Company']}</td><td>${row['Position/Title']}</td><td>${avgScore}</td><td>${riskIndex}%</td>`;
...
function renderPositionCards(data) {
    ...
    data.forEach(row => {
        ...
        positions[pos].risks.push(calcRisk(row));
        positions[pos].count++;
    });
    ...
    for (let pos in positions) {
        ...
        card.className = 'card';
        card.innerHTML = `<h3>${pos}</h3><p>Avg Score: ${avgScore}</p><p>Avg Risk: ${avgRisk}%</p><p>Respondents: ${positions[pos].count}</p>`;
...
function renderTrendCharts(data) {
    ...
    data.forEach(row => {
        ...
        grouped[date].risks.push(calcRisk(row));
    });
    ...
    new Chart(document.getElementById('trendScoreChart'), {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'Avg Score', data: avgScores, borderColor: '#4caf50' }] }
    });
    new Chart(document.getElementById('trendRiskChart'), {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'Risk Index', data: avgRisks, borderColor: '#f44336' }] }
    });
}
...
function calcRisk(row) {
    ...
    questionLabels.forEach(q => {
        const val = parseFloat(row[q]);
        if (!isNaN(val)) { count++; if (val <= 6) low++; }
    });
